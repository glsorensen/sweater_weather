...
RSpec.describe 'road trip requests' do
  describe 'happy path' do
    ...
    it 'will have all of the required attributes', :vcr do
        post "/api/v1/road_trip", params: {"origin": "Red Lodge, MT","destination": "Government Camp, OR", "api_key": "jgn983hy48thw9begh98h4539h4"}
    ...
    it 'will have the travel time', :vcr do
        post "/api/v1/road_trip", params: {"origin": "Red Lodge, MT","destination": "Government Camp, OR", "api_key": "jgn983hy48thw9begh98h4539h4"}
        response_info = JSON.parse(response.body, symbolize_names: true)
        expect(response_info[:data][:attributes][:travel_time]).to eq('')
    end

    it 'will have the weather & conditions at eta', :vcr do
        post "/api/v1/road_trip", params: {"origin": "Red Lodge, MT","destination": "Government Camp, OR", "api_key": "jgn983hy48thw9begh98h4539h4"}
        response_info = JSON.parse(response.body, symbolize_names: true)
        expect(response_info[:data][:attributes][:weather_at_eta][:temperature]).to eq()
        expect(response_info[:data][:attributes][:weather_at_eta][:conditions]).to eq()
    end
  end
  ...
  describe 'sad path' do
    ...
    it 'will return if the trip is impossible', :vcr do
        post "/api/v1/road_trip", params: {"origin": "Red Lodge, MT","destination": "London,UK", "api_key": "jgn983hy48thw9begh98h4539h4"}
        response_info = JSON.parse(response.body, symbolize_names: true)
